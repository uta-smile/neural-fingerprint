...
task_params = {'target_name': 'solub', 'data_file': 'data/solub.csv'}
N_train = 46284
N_val = 5785
N_test = 5786

model_params = dict(
    fp_length=200,  # Usually neural fps need far fewer dimensions than morgan.
    ...
    L2_reg=np.exp(-2))
train_params = dict(
    num_iters=30000, batch_size=32, init_scale=np.exp(-4), step_size=np.exp(-6))
...
def train_nn(pred_fun,
    ...
    grad_fun_with_data = build_batched_grad(
        grad_fun, train_params['batch_size'], train_smiles, train_targets)
