...
def sum_and_stack(features, idxs_list_of_lists):
    return fast_array_from_list(
        [np.sum(features[idx_list], axis=0) for idx_list in idxs_list_of_lists])
...
def build_convnet_fingerprint_fun(num_hidden_features=[100, 100],
    ...
    def output_layer_fun_and_atom_activations(weights, smiles):
        ...
        def write_to_fingerprint(atom_features, layer):
            ...
            atom_outputs = softmax(
                cur_out_bias + np.dot(atom_features, cur_out_weights), axis=1)
